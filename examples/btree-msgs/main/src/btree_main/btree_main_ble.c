...
static const uint16_t char_msg_uuid     = 0xFF01;
...
static const esp_gatts_attr_db_t gatt_db[GATTS_NUM_HANDLE] = {
    ...
    // MSG Characteristic Declaration
    [1] = 
    {{ESP_GATT_AUTO_RSP},
     {ESP_UUID_LEN_16, (uint8_t *)&char_declaration_uuid, ESP_GATT_PERM_READ,
      sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&char_prop}},

    // MSG Characteristic Value
    [2] = 
    {{ESP_GATT_AUTO_RSP},
     {ESP_UUID_LEN_16, (uint8_t *)&char_msg_uuid, ESP_GATT_PERM_WRITE,
      64, 0, NULL}},
};
                                ...
                                esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (handle == gatt_handle_table[2]) {
            btree_main_blackboard_set_buffer(BTREE_MAIN_BLACKBOARD_BUFFER_MSG, recv_buffer, recv_buffer_len);
            printf("MSG recebida: %.*s", recv_buffer_len, recv_buffer);
            set_bool_tick();
        } else {
